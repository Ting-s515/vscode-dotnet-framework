...
**方式 B：命令列（最精確，不會裝錯，可寫進 setup script）**

```powershell
code --install-extension ms-dotnettools.csharp
```

此指令直接用 Extension ID 安裝，不會誤觸其他同名套件；適合公司新人 onboarding script、CI / dotfiles 重建。

**方式 C：`.vsix` 離線安裝（公司網路擋 Marketplace 時的備案）**
...
3. 在公司電腦執行：
   ...
   或：VS Code → Extensions → 右上角 `...` 選單 → `Install from VSIX...` → 選檔。
